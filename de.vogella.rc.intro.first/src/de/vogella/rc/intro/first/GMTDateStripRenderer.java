...
/**
 * A simple TimeScaleRenderer that will always show the starting date. Modified to display the starting date in 
 * GMT rather the Kliems DateStripRenderer.
 * 
 * @author kliem
 * 
 */
public class GMTDateStripRenderer extends RendererBase implements TimeScaleRenderer {

    public GMTDateStripRenderer() {
        super(null);
    }

    public GMTDateStripRenderer(Printer printer) {
        super(printer);
    }
    ...
    public void draw(GC gc, Rectangle drawingArea, TimeBarViewerDelegate delegate, boolean top, boolean printing) {
        ...
        final SimpleDateFormat gmtDateFormat = new SimpleDateFormat(gmtFormatString);
        
        String date = gmtDateFormat.format(startDate.getDate());
       
        if (horizontal) {

            //clearbackground
            // TODO
            SwtGraphicsHelper.drawStringLeftAlignedVCenter(gc, date,
    ...
    public TimeScaleRenderer createPrintRenderer(Printer printer) {
        return new GMTDateStripRenderer(printer);
    }
